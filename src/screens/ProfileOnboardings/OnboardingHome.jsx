...
const OnboardingHome = () => {
  ...
  const getProfileDetailsStatus = async () => {
    try{
      ...
      if (getProfileResponse?.data !== null || getProfileResponse?.data !== undefined || getProfileResponse?.data !== ''){
  ...
  const getLoanUserDetailsStatus = async () => {
    try{
      ...
      if (getLoanUserDetailsResponse?.data !== null && getLoanUserDetailsResponse?.data !== undefined && getLoanUserDetailsResponse?.data  !== ''){
        if(getLoanUserDetailsResponse?.data?.organizationDetails !== '' && getLoanUserDetailsResponse?.data?.organizationDetails  !== null && getLoanUserDetailsResponse?.data?.organizationDetails  !== undefined){
          await AsyncStorage.setItem('hasStarted', '2');
        }
        if(getLoanUserDetailsResponse?.data?.nextOfKinDetails !== '' && getLoanUserDetailsResponse?.data?.nextOfKinDetails  !== null && getLoanUserDetailsResponse?.data?.nextOfKinDetails  !== undefined){
          await AsyncStorage.setItem('hasStarted', '3');
        }
        if(getLoanUserDetailsResponse?.data?.bankDetails !== '' && getLoanUserDetailsResponse?.data?.bankDetails  !== null && getLoanUserDetailsResponse?.data?.bankDetails  !== undefined){
          await AsyncStorage.setItem('hasStarted', '4');
        }   if(getLoanUserDetailsResponse?.data?.armUserBankDetails !== '' && getLoanUserDetailsResponse?.data?.armUserBankDetails  !== null && getLoanUserDetailsResponse?.data?.armUserBankDetails  !== undefined){
          await AsyncStorage.setItem('hasStarted', '5');
        }
        if(getLoanUserDetailsResponse?.data?.loanDocumentDetails !== '' && getLoanUserDetailsResponse?.data?.loanDocumentDetails  !== null && getLoanUserDetailsResponse?.data?.loanDocumentDetails  !== undefined){
          let counter = 0;
          for (const data in getLoanUserDetailsResponse?.data?.loanDocumentDetails){
            if (data !== "othersName" && data !== "validIdentificationType" && data !== "validIdentification"){
              if (getLoanUserDetailsResponse?.data?.loanDocumentDetails[data] !== '' && getLoanUserDetailsResponse?.data?.loanDocumentDetails[data] !== null && getLoanUserDetailsResponse?.data?.loanDocumentDetails[data] !== undefined){
                counter += 1;
              }
...
return (
  ...
  {hasStarted === '5' && (
    ...
    <TouchableOpacity
    onPress={() => navigation.navigate('ValidIdentity')}>
    <CustomView label="Add Documents" status={`[${hasDocumentCount} / 9] Filled`} />
    </TouchableOpacity>
